...
**Удаление и вставка** <a name="deletAndPaste"></a>
	...
	C - удаляет текст с текущего положения курсора до конца строк и переходит в режим ввода
...
Во всех файлах должен быть заголовок (header), который установлен правилами Школы (см.на рисунке ниже).
...
Чтобы скачаь Vundle, понадобится установить систему контроля версий Git (как им пользоваться, рассказываю  ниже):
...
Далее настраиваем Vim и пакетный менеджер Vundle, а также запишем в очередь на установку плагин 42header. Для этого создадим в каталоге ~/.vim файл vimrs:
