...
Где `origin` название сервера (удалённая ветка) онлайн-репозитория, `master` - ваша локальная ветка. Команда извлекает изменения `origin` и `master`, объединяет и записывает их в `master`. После выполнения команды, нам показывают, сколько изменений было добавлено и сколько удалено.
